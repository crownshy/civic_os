...
<div class={cn('shrink-0', skeleton && 'pointer-events-none opacity-40', className)}>
	...
	<div class="flex items-center gap-2.5 px-6 py-5">
		<Button
			{disabled}
			onclick={onAgree}
			class="min-w-0 flex-1 h-auto gap-1.5 rounded-[30px] px-5 py-4 shadow-[0px_4px_8.2px_0px_rgba(0,0,0,0.25)] disabled:cursor-not-allowed disabled:opacity-50"
		>
			<span>AGREE</span>
		</Button>
		<Button
		...
		<Button
			{disabled}
			onclick={onSkip}
			class="min-w-0 flex-1 h-auto rounded-[30px] bg-[#FFE9C8] px-5 py-4 text-secondary shadow-none disabled:cursor-not-allowed disabled:opacity-50"
		>
			<span>UNSURE</span>
		</Button>
	</div>
