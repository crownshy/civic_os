...
<nav class="flex items-stretch border-b border-foreground/30">
	<Popover.Root bind:open={switcherOpen}>
		<Popover.Trigger
			class="flex cursor-pointer items-center gap-3 bg-primary px-4 py-2.5 text-lg font-medium text-primary-foreground outline-none"
		>
	...
	{#each subTabs as tab (tab.href)}
		<a
			...
			class={`cursor-pointer px-4 py-2.5 text-lg font-medium ${
