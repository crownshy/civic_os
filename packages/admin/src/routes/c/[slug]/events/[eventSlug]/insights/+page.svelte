...
<div class="grid grid-cols-[1fr_320px] gap-4">
	...
	<div class="space-y-4">
		<div>
			<h3 class="text-base font-bold">Reports</h3>
			<p class="text-muted-foreground text-xs">appear once recordings are processed</p>
		</div>
		<div class="space-y-2">
			{#each reports as r}
				<div
				...
				>
					<div
						class={`flex size-12 items-center justify-center rounded-tl-xl rounded-tr-xl rounded-bl-2xl rounded-br-xl text-xs font-medium ${kindClass[r.kind] ?? 'bg-muted-foreground/20'}`}
					>
						{r.kind}
					</div>
					<div class="flex-1">
						<div class="text-sm font-bold">{r.title}</div>
						<div class="text-muted-foreground text-[10px]">{r.ts}</div>
					</div>
					<button
						type="button"
						class="border-foreground/20 size-8 rounded-tl-xl rounded-tr-xl rounded-bl-2xl rounded-br-xl border text-[10px]"
	...
	<!-- Right: recordings -->
	<div class="space-y-3">
		<h3 class="text-base font-bold">Add recordings</h3>
		<div
		...
		>
			...
			<div class="text-xs font-bold">drop .mp3 / .wav / .m4a</div>
		...
		<div class="space-y-1.5">
			{#each recordings as rec}
				<div class="bg-card shadow-card flex items-center gap-2 rounded-lg p-3 text-xs">
		...
		<div class="bg-card shadow-card space-y-1 rounded-lg p-3">
			<div class="text-xs font-bold">Phone recording</div>
			<div class="text-muted-foreground text-[10px]">Dial in & we'll transcribe.</div>
			<div class="bg-muted-foreground/10 mt-1.5 flex items-center gap-2 rounded-lg px-2.5 py-2">
				<span class="flex-1 text-sm">+1 (415) 555-0142</span>
				<span class="bg-foreground text-background rounded-full px-2.5 py-1 text-xs">pin 8417</span>
			</div>
		</div>
		<div class="bg-card shadow-card rounded-lg p-3 text-[10px]">
