...
<div class="p-6 sm:p-8">
	...
	<p class="text-muted-foreground mb-8 text-sm">Select a conversation to manage.</p>

	{#if data.conversations.length === 0}
		<p class="text-muted-foreground text-sm">No conversations configured.</p>
	{:else}
			...
			{#each data.conversations as conv (conv.slug)}
				...
				>
					<div class="flex items-start justify-between gap-2">
						<span
							class={`shrink-0 rounded px-1.5 py-0.5 text-xs font-semibold ${statusClass[conv.status]}`}
						>
					...
					<div class="min-w-0 flex-1">
						<p class="truncate text-base font-semibold leading-snug">{conv.title}</p>
						<p class="text-muted-foreground mt-0.5 text-sm">{conv.stateName}</p>
					</div>
