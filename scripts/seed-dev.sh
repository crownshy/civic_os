...
# then writes the resulting PUBLIC_DEV_* ids into both packages' .env files
# (packages/admin/.env and packages/civicos/.env).
#
...
# Both packages read the same PUBLIC_DEV_* vars — one .env per package, same
# values across both. Override with ENV_FILES="path1 path2" to pin them.
