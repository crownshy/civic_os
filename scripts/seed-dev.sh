...
# The Place this Campaign runs in. Its Campaigns are listed at /<place-slug>.
...
# have no slug, and /regions is authenticated while the participant app reads
# Places anonymously. So the Place rides on metadata for now, which is
...
# Which Campaign the site root redirects to. Participant URLs carry the slug
# on the path (/<org>/conversations/<slug>); this is only for bare \`/\`.
...
write_env() {
  ...
  if [ -f "$env_file" ]; then
    grep -v -e '^PUBLIC_DEV_' -e '^PUBLIC_CAMPAIGN_SLUG' -e '^# --- Local dev region (generated' -e '^# --- Generated by scripts/seed-dev.sh' -e '^# Which Campaign the Place root' -e '^# Which Campaign the site root' -e '^# on the path' "$env_file" > "$env_file.tmp" && mv "$env_file.tmp" "$env_file"
  fi
...
echo ""
echo "Then: pnpm dev → http://localhost:5173/host/conversations/$SEED_SLUG"
echo "      Place page → http://localhost:5173/$SEED_PLACE_SLUG"
echo ""
